...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("FortuneCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FortuneCoinTEST2\r");
            Send(hSocket, "WHO #FortuneCoinTEST2\r");
        } else {
            ...
            Send(hSocket, strprintf("JOIN #FortuneCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FortuneCoin%02d\r", channel_number).c_str());
        }
